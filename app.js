	...
	try{
		...
		app.version = "1.3.6"; // The version number of this code
		...
		app._init = function(){
			...
			// if there is an init() function in this app, call it
		...
		app._callInit = function(){
			// once everything is ready call init()
			if(app._scriptLoaded_jquery && app._scriptLoaded_ajaxq){
				app.init();
			}
			else
				setTimeout(app._callInit,500);
	...
	*/
		...
		app.initAccordion = function() {
			...
			// Thumb/ Image Links
			new Fx.Accordion(this, '.accordion.item.thumb', '.accordion.item.thumb .text', {
				alwaysHide: true,
				display:-1,
				onActive:function(t,el){
					el.getParent('.accordion.item.thumb').getElement('img').addClass('rotate');
				},	
				onBackground:function(t,el){
					el.getParent('.accordion.item.thumb').getElement('img').removeClass('rotate');
				}
			});
		...
		// Form object.
		// Use the name `AppForm` to avoid conflicts with other libraries using `Form` such as JS Validator
		...
		function Device(props) {
			...
			this.setRGB = function(color,pins){
				...
				app.setPin(pins.r,color.r);
				app.setPin(pins.g,color.g);
				app.setPin(pins.b,color.b);
